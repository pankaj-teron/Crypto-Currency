...
const Filter = () => {
  ...
  const handleCurrencyClick = (currency) => {
    setCurrency(currency); // Set the currency to either INR or USD
  };
  ...
  return (
    <div className="w-full lg:h-12 h-full lg:border-2 rounded-lg border-solid lg:border-gray-100 border-0 flex lg:flex-row flex-col lg:items-center lg:justify-between relative align-start justify-between">
      ...
      <div className="flex lg:mr-7 justify-between mt-4 lg:mt-0 sm:flex-row flex-col relative">
        <div className="relative flex items-center font-nunito mr-1 md:mr-12">
          <span className="mr-2 sm:text-base text-sm sm:font-bold font-medium">
            Currency:{" "}
          </span>
          <button
            onClick={() => handleCurrencyClick('USD')}
            className="w-13 h-7 rounded bg-gray-200 text-gray-700 px-2 py-1 mr-2
          ...
          <button
            ...
            className="w-12 h-7 rounded bg-gray-200 text-gray-700 px-2 py-1
            hover:bg-cyan-500 hover:text-white transition-all"
          >
            INR
          </button>
